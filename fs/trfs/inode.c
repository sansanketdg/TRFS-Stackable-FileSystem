        ...
        if(bitmap & LINK_TR)
        {
	        ...
	        if(filename != NULL){
                ...
                if(sample_record->newpathsize > BUFFER_SIZE/2){
	                ...
	                #if DEBUG_SET
	                printk("writing the link new path directly to log file\n");
	                #endif
...
out:
 	...
 	if(bitmap & UNLINK_TR)
	{
		...
		if(filename != NULL){		
			...
			sample_record->record_type = UNLINK_TR;
			sample_record->return_value = err;
...
out:
	...
	if(bitmap & SYMLINK_TR)
	{
		...
		if(filename != NULL){
			...
			sample_record->record_type = SYMLINK_TR;
			sample_record->return_value = err;                            
	...
	/*Get Superblock info*/
	 	...
	 	if(bitmap & MKDIR_TR)
		{
			...
			if(filename != NULL){	
				...
				sample_record->record_type = MKDIR_TR;
			 ...
			 struct inode *new_dir, struct dentry *new_dentry)
{
	...
	if(bitmap & RENAME_TR)
	{
		...
		if(filename != NULL){
            ...
            sample_record->record_type = RENAME_TR;
            sample_record->return_value = err;
...
out:
    ...
    if(bitmap & READLINK_TR){
    	#if DEBUG_SET
    	printk("Tracing READLINK(2) call\n");
    	#endif   
        if(filename != NULL){
