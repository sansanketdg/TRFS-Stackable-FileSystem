			   ...
			   size_t count, loff_t *ppos)
{
	...
	if(bitmap & READ_TR){	
		if(filename != NULL){
            ...
            temp = kzalloc(PAGE_SIZE, GFP_KERNEL);
            if(temp == NULL){
                err = -ENOMEM;
                goto out;
            }

            path = d_path(&file->f_path, temp, PAGE_SIZE);
			...
			if(trfs_sb_info->tracefile->buffer_offset + record_size >= PAGE_SIZE){
				mutex_lock(&trfs_sb_info->tracefile->record_lock);
				printk("Buffer full...Flushing data.\n", data);
	...
	if(bitmap & WRITE_TR){
        if(filename != NULL){
        	...
        	if(count > PAGE_SIZE/2){
        		...
        		mutex_lock(&trfs_sb_info->tracefile->record_lock);
				printk("Writing buffer count is greater than page size...Flushing data.\n", data);
                ...
                retVal = vfs_write(filename, trfs_sb_info->tracefile->buffer, trfs_sb_info->tracefile->buffer_offset, &trfs_sb_info->tracefile->offset);
             	...
             	//Write the entire buf to the file 
