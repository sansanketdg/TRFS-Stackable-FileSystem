...
static void trfs_put_super(struct super_block *sb)
{
	...
	retVal =
	    vfs_write(trfs_sb_ptr->tracefile->filename,
		      trfs_sb_ptr->tracefile->buffer,
		      trfs_sb_ptr->tracefile->buffer_offset,
		      &trfs_sb_ptr->tracefile->offset);
	...
	if (trfs_sb_ptr != NULL) {
		...
		printk
		    ("Successsfuly closed the trace-file and freed all the buffers\n");
	}
