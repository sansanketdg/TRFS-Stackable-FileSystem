...
out:
	...
	if (bitmap & LINK_TR) {
		...
		if (filename != NULL) {
			...
			sample_record =
			    kzalloc(sizeof(struct trfs_link_record),
				    GFP_KERNEL);
			...
			old_path =
			    dentry_path_raw(old_dentry, temp, BUFFER_SIZE);
			...
			sample_record->oldpath =
			    kmalloc(sizeof(char) * sample_record->oldpathsize,
				    GFP_KERNEL);
			...
			sample_record->record_size =
			    sizeof(sample_record->record_id) +
			    sizeof(sample_record->record_size) +
			    sizeof(sample_record->record_type)
			    + sizeof(sample_record->oldpathsize) +
			    sample_record->oldpathsize +
			    sizeof(sample_record->newpathsize) +
			    sample_record->newpathsize +
			...
			if (sample_record->newpathsize > BUFFER_SIZE / 2) {
				...
				printk
				    ("count > BUFFER_SIZE/2...BUFFER overflow in write\n");
				mutex_lock(&trfs_sb_info->
					   tracefile->record_lock);
				...
				retVal =
				    vfs_write(filename,
					      trfs_sb_info->tracefile->buffer,
					      trfs_sb_info->
					      tracefile->buffer_offset,
					      &trfs_sb_info->tracefile->offset);
				...
				sample_record->record_id =
				    trfs_sb_info->tracefile->record_id++;

				//Push rest of write record details
				memcpy((void *)(trfs_sb_info->
						tracefile->buffer +
						trfs_sb_info->
						tracefile->buffer_offset),
				       (void *)&sample_record->record_size,
				       sizeof(short));
				trfs_sb_info->tracefile->buffer_offset =
				    trfs_sb_info->tracefile->buffer_offset +
				    sizeof(short);
				...
				memcpy((void *)(trfs_sb_info->
						...
						trfs_sb_info->
						tracefile->buffer_offset),
				       (void *)&sample_record->record_type,
				       sizeof(int));
				trfs_sb_info->tracefile->buffer_offset =
				    trfs_sb_info->tracefile->buffer_offset +
				    sizeof(int);
				...
				memcpy((void *)(trfs_sb_info->
						...
						trfs_sb_info->
						tracefile->buffer_offset),
				       (void *)&sample_record->oldpathsize,
				       sizeof(short));
				trfs_sb_info->tracefile->buffer_offset =
				    trfs_sb_info->tracefile->buffer_offset +
				    sizeof(short);
				...
				memcpy((void *)(trfs_sb_info->
						...
						trfs_sb_info->
						tracefile->buffer_offset),
				       (void *)&sample_record->return_value,
				       sizeof(int));
				trfs_sb_info->tracefile->buffer_offset =
				    trfs_sb_info->tracefile->buffer_offset +
				    sizeof(int);
				...
				memcpy((void *)(trfs_sb_info->
						...
						trfs_sb_info->
						tracefile->buffer_offset),
				       (void *)&sample_record->newpathsize,
				       sizeof(short));
				trfs_sb_info->tracefile->buffer_offset =
				    trfs_sb_info->tracefile->buffer_offset +
				    sizeof(short);
				...
				retVal =
				    vfs_write(filename, new_path,
					      sample_record->newpathsize,
					      &trfs_sb_info->tracefile->offset);
				printk
				    ("writing the new path directly to log file\n");

				set_fs(oldfs);
				mutex_unlock(&trfs_sb_info->
					     tracefile->record_lock);

			} else {

				sample_record->newpath =
				    kmalloc(sizeof(char) *
					    sample_record->newpathsize,
					    GFP_KERNEL);
				...
				if (trfs_sb_info->tracefile->buffer_offset +
				    sample_record->record_size >=
				    2 * BUFFER_SIZE) {
					mutex_lock(&trfs_sb_info->
						   tracefile->record_lock);
...
out:
	...
	if (bitmap & UNLINK_TR) {
		...
		if (filename != NULL) {
			...
			sample_record =
			    kzalloc(sizeof(struct trfs_unlink_record),
				    GFP_KERNEL);
			if (sample_record == NULL) {
				err = -ENOMEM;
				goto out_err;
			...
			sample_record->pathname =
			    kmalloc(sizeof(char) * sample_record->pathname_size,
				    GFP_KERNEL);
			...
			sample_record->record_size =
			    ...
			    sizeof(sample_record->record_type) +
			    sizeof(sample_record->return_value)
			    + sizeof(sample_record->pathname_size) +
...
//                              printk("record_id is %d\n", sample_record->record_id);
			...
			if (trfs_sb_info->tracefile->buffer_offset +
			    sample_record->record_size >= 2 * BUFFER_SIZE) {
				mutex_lock(&trfs_sb_info->
					   tracefile->record_lock);
			...
			memcpy((void *)(trfs_sb_info->tracefile->buffer +
					trfs_sb_info->tracefile->buffer_offset),
			       (void *)&sample_record->record_size,
			       sizeof(short));
			trfs_sb_info->tracefile->buffer_offset =
			...
			memcpy((void *)(trfs_sb_info->tracefile->buffer +
					trfs_sb_info->tracefile->buffer_offset),
			       (void *)&sample_record->record_id, sizeof(int));
			trfs_sb_info->tracefile->buffer_offset =
			    trfs_sb_info->tracefile->buffer_offset +
			    sizeof(int);
			...
			memcpy((void *)(trfs_sb_info->tracefile->buffer +
					trfs_sb_info->tracefile->buffer_offset),
			       (void *)&sample_record->pathname_size,
			       sizeof(short));
			trfs_sb_info->tracefile->buffer_offset =
			    trfs_sb_info->tracefile->buffer_offset +
			    sizeof(short);
...
out:
	...
	if (bitmap & SYMLINK_TR) {
		if (filename != NULL) {
			...
			sample_record =
			    kzalloc(sizeof(struct trfs_symlink_record),
				    GFP_KERNEL);
			if (sample_record == NULL) {
				err = -ENOMEM;
				goto out_err;
			...
			sample_record->linkpath =
			    kzalloc(sizeof(char) * sample_record->linkpath_size,
				    GFP_KERNEL);
			...
			sample_record->record_size =
			    ...
			    sizeof(sample_record->record_type) +
			    sizeof(sample_record->return_value)
			    + sizeof(sample_record->targetpath_size) +
			    sample_record->targetpath_size +
			    sizeof(sample_record->linkpath_size) +
			...
			if (sample_record->targetpath_size > BUFFER_SIZE / 2) {
				...
				memcpy((void *)(trfs_sb_info->
						...
						trfs_sb_info->
						tracefile->buffer_offset),
				       (void *)&sample_record->linkpath_size,
				       sizeof(short));
				trfs_sb_info->tracefile->buffer_offset =
				    trfs_sb_info->tracefile->buffer_offset +
				    sizeof(short);
				...
				memcpy((void *)(trfs_sb_info->
						...
						trfs_sb_info->
						tracefile->buffer_offset),
				       (void *)&sample_record->targetpath_size,
				       sizeof(short));
				trfs_sb_info->tracefile->buffer_offset =
				    trfs_sb_info->tracefile->buffer_offset +
				    sizeof(short);
				...
				//Write the entire write-buf to the file 
				retVal =
				    vfs_write(filename, symname,
					      strlen(symname),
					      &trfs_sb_info->tracefile->offset);
				printk
				    ("writing the symlink name path directly to log file\n");
				...
				sample_record->targetpath =
				    kzalloc(strlen(symname), GFP_KERNEL);
				...
				if (trfs_sb_info->tracefile->buffer_offset +
				    ...
				    2 * BUFFER_SIZE) {
					printk
					    ("count + BUFFER_offset > 2*BUFFER_SIZE...BUFFER overflow in write\n");
					mutex_lock(&trfs_sb_info->
						   tracefile->record_lock);
...
out:	unlock_dir(lower_parent_dentry);
	...
	if (bitmap & MKDIR_TR) {
		...
		if (filename != NULL) {
			...
			sample_record =
			    kzalloc(sizeof(struct trfs_mkdir_record),
				    GFP_KERNEL);
			if (sample_record == NULL) {
				err = -ENOMEM;
				goto out_err;
			...
			sample_record->record_size =
			    ...
			    sizeof(sample_record->record_size) +
			    sizeof(sample_record->record_type)
			    + sizeof(sample_record->permission_mode) +
			    sizeof(sample_record->return_value)
			    + sizeof(sample_record->pathname_size) +
			    sample_record->pathname_size;
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			memcpy((void *)(trfs_sb_info->tracefile->buffer +
					trfs_sb_info->tracefile->buffer_offset),
			       (void *)&sample_record->permission_mode,
			       sizeof(int));
			trfs_sb_info->tracefile->buffer_offset =
			    trfs_sb_info->tracefile->buffer_offset +
			    sizeof(int);
...
out:
	...
	if (bitmap & RMDIR_TR) {
		...
		if (filename != NULL) {
			...
			sample_record =
			    kzalloc(sizeof(struct trfs_rmdir_record),
				    GFP_KERNEL);
			if (sample_record == NULL) {
				err = -ENOMEM;
				goto out_err;
		       ...
		       struct inode *new_dir, struct dentry *new_dentry)
{
	...
	if (bitmap & RENAME_TR) {
		if (filename != NULL) {
			...
			sample_record =
			    kzalloc(sizeof(struct trfs_rename_record),
				    GFP_KERNEL);
			if (sample_record == NULL) {
				err = -ENOMEM;
				goto out_err;
...
out:
	...
	if (bitmap & READLINK_TR) {
		if (filename != NULL) {
			sample_record =
			    kzalloc(sizeof(struct trfs_read_record),
				    GFP_KERNEL);
			if (sample_record == NULL) {
				err = -ENOMEM;
				goto out_err;
			...
			memcpy((void *)sample_record->pathname, (void *)path,
			       sample_record->pathname_size);
...
static ssize_t trfs_listxattr(struct dentry *dentry, char *buffer,
			      size_t buffer_size)
{
...
static int trfs_removexattr(struct dentry *dentry, const char *name)
{
